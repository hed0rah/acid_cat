...
| `acidcat tui [FILE]` | Interactive terminal inspector: goto/search, follow pointers (`x`), byte map (`m`), edit fields, and validate (`v`, which offers repair). Graph views with `b`, scoped to the selected chunk with `r` and rescaled with `S`; `z` gives a pane the whole screen, `?` lists every key. Omit FILE to open the built-in file browser |
