...
| `acidcat tui [FILE]` | Interactive terminal inspector: goto/search, follow pointers (`x`), byte map (`m`), edit fields, and validate (`v`, which offers repair). Graph views with `b`; focus one and the arrows drive it (up/down rescale, left/right widen to the file or narrow to the selected chunk), or use `S` and `r`. `z` gives a pane the whole screen, `?` lists every key. Omit FILE to open the built-in file browser |
