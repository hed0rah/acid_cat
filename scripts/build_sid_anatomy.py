...
BODY = """<div class="sheet">
  ...
  <div class="intro">
    ...
    routine at interrupt rate while the code writes the SID chip's registers itself. The sound is
    a side effect of execution, not a payload that can be decoded.</p>
  ...
  <div class="sec">the byte-order trap</div>
  ...
  native little-endian. One file, read in both directions, with the boundary at exactly one
  offset.</p>
  ...
  <div>
    <details class="region" open>
      ...
      <div class="rbody">
        <p class="note">Not two formats. <b>RSID</b> is <b>PSID</b> with four fields pinned and a
    ...
    <details class="region">
      <summary><span class="chev">&#9656;</span><span class="rname">where the load address really lives</span><span class="rspan">0x08 . 2 bytes</span></summary>
      <div class="rbody">
    ...
    <details class="region">
      ...
      <div class="rbody">
        ...
        the CIA 1 timer. Past 32 subtunes the two header generations genuinely disagree, and only
    ...
    <details class="region">
      ...
      <div class="rbody">
        ...
        <p class="note">The two gaps are not arbitrary. 0x00-0x41 would collide with the first SID
    ...
    <details class="region">
      ...
      <div class="rbody">
        <p class="note">A SID header describes a machine as much as a file. Loading the image is
        not enough; the machine around it has to be in a defined state or the tune's timing is
    ...
    <details class="region">
      ...
      <div class="rbody">
        ...
        are small enough that volume-register samples need a hardware or software trick. And the
        two analog filters have entirely different characteristics, so a filter sweep written for
...
BUILDS = r"""  // v1 header, the fixed numeric block: 0x00 - 0x15
  build("sid-head","byte",
    ...
    [
      ...
      {label:"loadAddress",k:"enum",r:[8,9],val:"$0000",sel:0,branch:[
       ...
       note:"the single most misread field in the format."},
  ...
  build("sid-flags","bit",[0x00,0xA4],[
    ...
    {label:"musPlayer",k:"flag",r:[15,15],val:"0",sel:0,branch:[
     ...
     note:"changes what the payload IS, not just how it sounds."}
  ]);
