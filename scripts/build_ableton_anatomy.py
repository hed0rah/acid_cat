...
<div class="sheet">
  ...
  <div class="panelwrap">
    ...
    <div class="panel" data-panel="grid">
      ...
      <div class="callout"><b>This is the reason to walk the format.</b> An orphaned <code>.asd</code> in a folder is evidence about a file that no longer exists -- its rate, its length, its exact sample count. Nothing else in the toolchain recovers that, because nothing else survives the audio.</div>
      <div class="callout"><b>No tempo NUMBER is stored, but the tempo is recoverable.</b> The value appears nowhere -- at no byte offset, as neither f32 nor f64, and in no derived form. Live keeps a mapping instead: warp markers pin seconds to beats, and the tempo falls out of any two of them. See the Warp tab.</div>
    </div>
    ...
    <div class="panel" data-panel="objects">
      ...
      <p class="plede">After the grid comes a serialised object tree that <b>writes its own field names inline</b>. They are length-prefixed <b>UTF-16</b>, so every second byte is zero and an ASCII string scan returns nothing -- which is most of why this format has a reputation for being opaque. The names are not gathered in a header; they are <b>interleaved with their data</b> across the whole file, so a parser can seek a field by name instead of by a version-specific offset.</p>
      ...
      <div class="callout"><b>Declared is not stored.</b> This schema is shared with the Live Set, so it names things the sidecar never carries. <code>LoopStart</code>, <code>LoopEnd</code>, <code>IsWarped</code>, <code>WarpMode</code>, <code>Numerator</code> and <code>Denominator</code> are all declared here -- and <code>LoopEnd</code> is <b>absent from 91%</b> of files whose Set states a real one. Loop points belong to a CLIP, and one audio file can back many clips with different loops. What the sidecar holds is per-file ANALYSIS.</div>
    </div>
    ...
    <div class="panel" data-panel="warp">
      ...
      <p class="plede">A warp marker pins one instant of the recording to one position in the bar. Each record carries its own class name inline, u8-length-prefixed, so the markers are <b>self-locating</b> -- there is no count to find and no offset to derive.</p>
    ...
    <div class="panel" data-panel="onsets">
      ...
      <div class="callout"><b>Two traps, both paid for.</b> The arrays are <b>not</b> word-aligned to the end of the frame grid, so a 4-stepped scan walks straight past them. And a count of 1 must be refused: with a single position "strictly increasing" constrains nothing, so any stray pair of equal u32 satisfies the test -- accepting it gave 7 of 14 files a confidently wrong answer. A genuine one-shot is reported as unknown rather than guessed at.</div>
      <div class="callout"><b>Onsets are not warp markers.</b> Live snaps markers to detected transients, so a marker frequently lands exactly on an onset -- 12 of 57 across five files. The counts are what separate them: one file carries 14 onsets and 9 markers. Reading one as the other looks convincing and is wrong.</div>
    </div>
    ...
    <div class="panel" data-panel="liveset">
      ...
      <div class="callout"><b>A Set and a Clip are indistinguishable by content.</b> Both open <code>&lt;LiveSet&gt;</code>. Only the extension separates them. There is no content rule that tells them apart, so any reader has to go by the name and say that it is doing so.</div>
...
0x18  "ptch"  u32:len      mx@c ... { patcher JSON }</pre>
  ...
  <footer>
    <span>acidcat / ableton anatomy</span>
    <span>asd . als / alc / adg / adv / agr . amxd</span>
    <span><a href="https://hed0rah.github.io">hed0rah</a> &middot; <a href="https://x.com/r3l0z">r3l0z</a></span>
  </footer>
</div>
"""
