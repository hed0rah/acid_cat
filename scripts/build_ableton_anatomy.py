    ...
    "asd-warp":["byte",
      ...
      [
        ...
        {label:"SecTime",k:"flag",r:[15,22],val:"0.011029411764705883",
          ...
          note:"a 17-significant-digit double. Finding it bit for bit is what proved the layout."},
    ...
    "asd-onsets":["byte",
      ...
      [
        ...
        {label:"positions[0]",k:"enum",r:[4,7],val:"1,491",
          ...
          note:"validated against the frame count the grid already gave us."},
  ...
  <div class="panelwrap">
    ...
    <div class="panel on" data-panel="overview">
      ...
      <table class="tbl">
        ...
        <tr><td><code>.agr</code></td><td>groove / quantize</td><td>gzip + XML <code>&lt;Groove&gt;</code></td><td>3</td><td>yes</td></tr>
        </tbody>
      </table>
      <div class="callout"><b>Live has more document types than these.</b> <code>.ams</code> (Operator meta sound), <code>.abl</code> and <code>.ablbundle</code> (Note) and <code>.ask</code> (theme) are the same gzip + <code>&lt;Ableton&gt;</code> XML shape. None occur in the library this was built from, so rather than guess at them acidcat reports the ROOT CHILD it found &mdash; the element that names the type. An unmapped document says what it is instead of being called a device preset.</div>
    ...
    <div class="panel" data-panel="grid">
      ...
      <div class="callout"><b>No tempo NUMBER is stored, but the tempo is recoverable.</b> Searching 599 files whose own filenames state their BPM finds the value absent &mdash; at every byte offset, as f32 and f64, and in derived forms. Live keeps a mapping instead: warp markers pin seconds to beats, and the tempo falls out of any two of them. See the Warp tab.</div>
    </div>
    ...
    <div class="panel" data-panel="warp">
      ...
      <div class="callout"><b>The tempo is the mapping, not a number.</b> Nothing in a <code>.asd</code> stores a BPM &mdash; searching 599 files whose own filenames state their tempo finds the value absent, at every byte offset, as f32 and f64, and in derived forms. But two markers give it directly: <code>(beats / seconds) x 60</code>. Here 0.03125 beats in 0.011029411764705883 s is <b>170 BPM</b>, and the Live Set declares 170. Across 40 clips the derivation matched the declared tempo every time, and over a 2,000-file sample it yields only clean musical values: 140, 170, 138, 120, 150, 145, 70, 172.</div>
      <div class="callout"><b>Most files have none, and that is not a gap.</b> Markers appear in about 3% of a sample library, because a one-shot nobody warped has nothing to pin. Of 879 clips a Live Set marked unwarped, <b>879 had no markers</b>. The presence of this section is not a format version: it appears under Live 9.7 through 12, and one project written by a single version contains sidecars both with and without it &mdash; a <code>.asd</code> is written when the audio is analysed, and outlives the projects that use it.</div>
    </div>
